...
fn(state => {
  ...
  const data = state.medication_ids
    .flatMap(medicine => {
      ...
      return state.obat
        .filter(item => item.fields['Nama'] === medicineName)
        .map(item => ({
          ...item,
          fields: { ...item.fields, satusehat_id: Object.values(medicine)[0] },
        }));
    })
    .map(obat => {
      const data = {
...
bulk('lookup-table', state => state.medicationMappedData, { replace: false });
