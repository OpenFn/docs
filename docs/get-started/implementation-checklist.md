...
their upcoming implementation. **The implementation process is broken up into
the seven phases summarized below. Check out the checklist for detailed steps.**
...
Iterate on workflow requirements to define technical specifications for how the
workflow will be implemented. For instance, consider specific API endpoints to
access and HTTP methods/operations to use for each.
...
Configure the workflow on OpenFn.org and develop and test the jobs and adaptors
...
- Drafted “Project Security Configuration Checklist” to document config settings
  implemented

## (5) Testing

Create a test suite and conduct UAT. After UAT, Incorporate any feedback and
iterate on the testing process.
...
Train OpenFn administrators and target system end users and document what was
implemented. This is also the phase where the configuration and code is migrated
...
Turn “on” OpenFn workflows to go-live and establish support structures & a
governance model for change management.
