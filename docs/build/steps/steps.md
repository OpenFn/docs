...
- `Adaptor` - the adaptor that is used to provide tool-specific functionality
  for this step (e.g., `language-dhis2` or `language-commcare`)
- `Credentials` -
- `Input` -
- `Output` -
...
will be determined by the triggering webhook request (the value inside `form.surname`,
...
present at `state.data.form.surname` for the triggering webhook request. It might be Bob
